...
int main(int argc, char* argv[]) {
  int valid_colors = 0;
  if(argc < 2) {
    cout << "ERR_MISSING: One or more RGB values should be provided as arguments, sepertated by spaces" << endl;
    return 0;
  }
...
bool all_hex(char* word) {
  for(int i=0; i<strlen(word); i++) {
    if((word[i] >= '0' && word[i] <= '9') || (word[i] >= 'a' && word[i] <= 'f')
      ...
      cout << word[i] << " ERR_VALUE: RGB values must be in the 0-F range" << endl;
      return false;
    }
  }
